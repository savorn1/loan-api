...
- `loan-service` → `accounting-service`: `AccountingClient` (`POST /api/journal-entries/generate`, `POST /api/journal-entries/{id}/system-reverse`) — `LoanServiceImpl.recordTransaction` calls `generate` for every DISBURSEMENT/PRINCIPAL_PAYMENT/INTEREST_PAYMENT/PENALTY_PAYMENT/FEE_PAYMENT/WRITE_OFF it logs (ADJUSTMENT/SETTLEMENT have no accounting-service `TransactionType` equivalent and are skipped); `voidDisbursement` calls `system-reverse` on the `journalEntryId` `approveDisbursement` stored on the `LoanDisbursement` row.
- `accounting-service` → `loan-service`: `LoanClient` (`GET /api/loans/reports/portfolio-summary`) — used only by `ReconciliationServiceImpl` to compare the GL's Loans Receivable control account against loan-service's own outstanding-balance total.
...
`loan-service` owns `loans` (the entity itself carries the full PENDING → APPROVED/REJECTED → ACTIVE → CLOSED lifecycle) plus a separate pre-approval workflow: `loan_applications` (`Application` entity — named to avoid colliding with the module's own Spring Boot bootstrap class, `com.example.loan.LoanApplication`) with child tables `loan_application_documents`, `loan_application_notes` and `loan_application_approvals`. A customer submits an `Application` (SUBMITTED); once an `ApplicationApproval` row with decision `APPROVED` is recorded (`POST /api/loans/applications/{id}/approvals`), `ApplicationServiceImpl` creates the actual `Loan` directly in `APPROVED` status — using the approver's `approvedAmount`/`approvedInterestRate`/`approvedTermMonths`, which may differ from what was originally requested — and links it back via `Application.loanId`, skipping `Loan`'s own redundant `PENDING`/`approve()` step. A `REJECTED` decision just marks the application `REJECTED`; no loan is created.

`loan-product-service` owns `loan_products` (product catalog: amount/term ranges, default interest rate, currency, effective date range) plus five child tables scoped to a product — `loan_product_interest_rates`, `loan_product_fees`, `loan_product_terms`, `loan_product_rules` (structured eligibility conditions: field/operator/value), `loan_product_documents` (a required-document checklist, no file storage). Unlike every other entity in this monorepo, `LoanProduct.id` is a `UUID` (`GenerationType.UUID`), not a `Long` identity — that was an explicit schema choice for this table only; its child tables still use `Long` identity ids and carry a `loan_product_id` FK. It has no Feign clients and nothing else calls it yet.

`accounting-service` implements the double-entry books: `gl_accounts` (chart of accounts, self-referencing `parent_id` hierarchy), `journal_templates` + `journal_template_lines` (per-`TransactionType` blueprints of symbolic debit/credit "account roles"), `accounting_schemes` (binds a template's role + currency to a real `gl_account`), `financial_periods` (OPEN/CLOSED date ranges that every posting is scoped to), and `journal_entries` + `journal_entry_lines` (the actual DRAFT → POSTED → REVERSED double-entry postings, validated balanced on both create and post). `TransactionType` (DISBURSEMENT, PRINCIPAL_PAYMENT, INTEREST_PAYMENT, etc.) is the only vocabulary this service shares with loan-service/payment-service — it has no notion of a loan product; upstream services can either call `POST /api/journal-entries` directly with fully-resolved lines and an opaque `referenceType`/`referenceId` pointing back at their own domain object, or `POST /api/journal-entries/generate` to have this service resolve `transactionType` → `JournalTemplate` → `AccountingScheme` itself and post immediately (see `JournalEntryServiceImpl.generate`; idempotent on `(transactionType, referenceType, referenceId)`). `GlAccountSeeder` and `JournalTemplateSeeder` (`@Order`ed 1 and 2) seed a default USD chart of accounts + one two-line template per generate()-able TransactionType on first boot. General ledger (`GET /api/gl-accounts/{id}/ledger`) and trial balance (`GET /api/trial-balance`) are read-only reports computed from `journal_entry_lines`, not separately stored. `GET /api/reconciliation/loans-receivable` compares GL account 1100 against loan-service's portfolio total (see Feign clients above) — a mismatch means either a bug, a missed accounting call, or a manual journal entry that bypassed loan-service; loans disbursed/paid before this integration existed will show as permanent variance unless backfilled.
