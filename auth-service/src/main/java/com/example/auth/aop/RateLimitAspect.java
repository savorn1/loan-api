...
// Mirrors customer-service's RateLimitAspect — same annotation-driven, Redis-backed,
// per-IP counter. First use here: throttling /api/auth/login, which previously had no
// protection against scripted password guessing at all.
@Aspect
@Component
...
public class RateLimitAspect {
    ...
    public Object enforce(ProceedingJoinPoint pjp, RateLimit rateLimit) throws Throwable {
        ...
        if (count != null && count > rateLimit.max()) {
            ...
            throw new TooManyRequestsException("Too many requests. Try again in " + ttl + " second(s).");
        }
        return pjp.proceed();
    }
